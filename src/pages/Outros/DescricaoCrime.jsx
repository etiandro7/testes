...
export default function DescricaoCrime({ onFechar = () => {}, id = 'modal' , Tip }) {
    const [descricao, setdescricao] = useState('')
    const [Csel, SetCsel] = useState(Tip)

    function Salvar() {
        fetch('../api/Outros/Descricao',
            {
                method: 'POST',
                body: JSON.stringify({ descricao, Csel })
            })
    }
    ...
    return (
      ...
      <div className="h-auto w-1/2 bg-white opacity-100">
                ...
                <h1 className={`relative bg-blue-900 h-10 py-3 font-extrabold pl-1 shadow-inner text-gray-50`}>REGISTO DE MODALIDADE CRIMINAL
                <span
                ...
                <Entrada label="Descrição da modalidade"
                    valor={descricao}
                    valorMudou={setdescricao}
                    texto="Digite a Modalidade Criminal"
                />
              </div>
