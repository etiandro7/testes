...
export default function TipodeArma({ onFechar = () => {}, id = 'modal' }) {
    const [nomearma, setnomearma] = useState('')
    function Salvar() {
        fetch('../api/Outros/TipodeArma', {
            method: 'POST',
            body: JSON.stringify({ nomearma }),
        })
    }
    ...
    return (
      ...
      <div className="h-auto w-1/2 bg-white opacity-100">
                ...
                <hr />
                <Entrada
                    label="Descrição do Objecto"
                    valor={nomearma}
                    valorMudou={setnomearma}
                    texto="Descrição do Objecto"
                />
